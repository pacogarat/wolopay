<?php
namespace AppBundle\Payment\PayMethod\PaySafeCard\Lib;
...
class SOPGClassicMerchantClient
{
    ...
    public function __construct($debugStatus='', $sysLang='', $autoCorrect='', $status='', $logger) {
        $this->logger = $logger;
    ...
    private function addLog($msg_code,$call,$call_params='_null_',$result='_null_',$type='error')
    {
        if($type != 'info')
        {
            ...
            $this->logger->addInfo("call: $call, callparams: $call_params, msg code: $msg_code");
        }
        else
        {
            $this->log['info'] = array('msg_code' => $msg_code);
            $this->logger->addInfo($msg_code);
        }
