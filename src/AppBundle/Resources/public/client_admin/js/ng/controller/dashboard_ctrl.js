smartApp.controller('DashboardController', ['$filter', 'APIStats', '$scope', '$rootScope', 'Sparkline', 'Flot', '$translate', 'Utils', '$timeout', 'tableBelow', 'Permissions', '$animate', 'CsvUtil', 'LoadMoreScroll', '$log',
    function ($filter, APIStats, $scope, $rootScope, Sparkline, Flot, $translate, Utils, $timeout, tableBelow, Permissions, $animate, CsvUtil, LoadMoreScroll, $log) {

        $scope.tableSummaryWasExecuted = false;
        $scope.jsonSparkLine = {
            chart: {
                height: 30,
                width: 82,
            ...
            tooltip: {
                enabled: true,
                headerFormat: "",
                pointFormat: "<b>{point.y}"+$rootScope.currency.symbol+"</b>",
                positioner: function (w, h, point) {
                    return { x: point.plotX - w , y: point.plotY + 10 };
                }
            }
        };

        $scope.jsonSparkLine2 = {
            chart: {
                height: 30,
                width: 82,
                tooltip: {
        ...
        var loadMoreScrollHandler = LoadMoreScroll.onScrollToElement('#wid-id-dashboard_dashboard', function(callback){ $scope.tableSummaryLoad(callback); });
        ...
        $scope.tableSummaryLoad = function(callback){
            callback = callback || function(){};

            $scope.tableSummaryWasExecuted = true;
            $log.debug('Executed tableSummaryLoad');
            var payMethods = {};
            APIStats.getPayMethodsByGames($scope.showPayMethods).success(function (data){
                    ...
                    angular.forEach(data, function(collection) {
                        ...
                        if ($scope.showPayMethods)
                        {
                            payMethods[collection.date].apps[collection.app_id].countries[collection.country_iso ].pay_methods[collection.pay_method] += {
                                ...
                                amount_tax: collection.amount_tax,
                ...
                angular.forEach(tablePayMethods, function(row) {
                    ...
                    tableReviewTotals.amount_tax += row.amount_tax;
    ...
    var exe = function exe(){
        ...
        APIStats.getAll().success(function (data){
            ...
            $translate(['revenue', 'transactions', 'purchases', 'game_amount', 'refunds']).then(function(translations){
                ...
                $timeout(function() {
                    ...
                    $scope.stats = data;
                    $scope.amountsTotalSpark = Flot.parseArrayKey(data.amounts_total_pos, 'static');
                    $scope.amountsGameSpark = Flot.parseArrayKey(data.amounts_game, 'static');
                    $scope.amountsTaxesSpark = Flot.parseArrayKey(data.amounts_taxes, 'static');
                    $scope.amountsRefundsSpark = Flot.parseArrayKey(data.refundsAmount_sum, 'static');
                    ...
                    angular.forEach(data.apps, function(app, key) {
                        ...
                        dataStats.push({
                            ...
                            fillOpacity: 0.30,
                            stacking: 'normal',
                            stack: 'transactionsRefunded',
                            name: appName+", "+translations.refunds,
                            data:  Flot.parseArrayKey(app.refundsNumber, dateFormat, null,-1),
                            color: Flot.increaseBrightness(color, 20),
                            name_custom: appName
                        });
                        revenuePie.push({
                            name: appName,
                            y: Math.round(app.amounts_game_pos_sum * 100)/100,
                            color: Flot.increaseBrightness(color, 20),
                            name_custom: appName
                        });
                        refundsPie.push({
                            name: appName,
                            y: Math.round(app.amounts_total_neg_sum * -100)/100,
                            color: Flot.increaseBrightness(color, 20),
                            name_custom: appName
                        });
                    ...
                    dataStats.push({
                        type: 'pie',
                        name: translations.refunds,
                        borderColor: "#777777",
                        data: Flot.sortDataPieCharts(refundsPie),
                        tooltip: { valueSuffix: $rootScope.currency.symbol },
                        center: ["97%", 30],
                        innerSize: '30%',
                        size: 50,
...
//                    $scope.gameAmountTable = tableBelow.createObjectByBarChart(temp, true, $rootScope.currency.symbol);
                    ...
                    angular.forEach(data.apps, function(app, key) {
                        ...
                        dataStats.push({
                            ...
                            data:  Flot.parseArrayKey(app.amounts_game_pos, dateFormat),
                            color: color,
                            name_custom: appName
                        });
                        dataStats.push({
                            ...
                            stack: 'revenue',
                            name: appName+", "+translations.refunds,
                            data:  Flot.parseArrayKey(app.refundsAmount, dateFormat),
                            color: color,
                            name_custom: appName
                        });
                    });
                        ...
                        series.push(
                            {
                                ...
                                stacking: 'normal',
                                stack: 'counts',
                                name: translations['purchases']+' '+affiliateName,
    ...
    //                }, 1000);
        ...
        if ($scope.tableSummaryWasExecuted)
        ...
        $scope.tableReviewToCSV = function()
        {
            ...
            'dashboard.table.game_total', 'tax_total','n_unique_users_by_purchase','dashboard.table.tax_total'
                ...
                CsvUtil.addToTemp(translations['dashboard.table.tax_total']);
                        ...
                        angular.forEach(app.countries, function(country) {
                            ...
                            if ($scope.showPayMethods)
                            {
                                angular.forEach(country.pay_methods, function(pay_method) {
                                    ...
                                    CsvUtil.addToTemp($rootScope.currency.id);
                                    ...
                                    CsvUtil.addToTemp(pay_method.amount_tax, 'num');
                                ...
                                CsvUtil.addToTemp(country.amount_tax, 'num');
    ...
    function sum(obj, valuesObj)
    {
        ...
        obj.amount_tax+= valuesObj.amount_tax;
