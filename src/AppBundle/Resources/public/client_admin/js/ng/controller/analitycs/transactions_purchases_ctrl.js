smartApp.controller('AnalyticsTransactionsPurchasesController', ['APIStats', '$scope', '$rootScope', '$log', 'Sparkline', 'Flot', '$translate', '$timeout',
    function (APIStats, $scope, $rootScope, $log, Sparkline, Flot, $translate, $timeout, CsvUtil) {
        ...
        $translate(['revenue', 'transactions', 'purchases', 'game_amount', 'analitycs.revenue_by_single_payments',
            ...
            var exe = function exe()
            {
                ...
                APIStats.getTransactionPurchases().success(function (data){
                    ...
                    var tableTotal = {purchases:0,transactions:0,unique_users:0,amount_game_vs_unique_users:0,amount_total:0,amount_game:0, n_unique_users_by_purchase: 0};

                    angular.forEach(data.transactions_full, function(collection) {
                        collection.cr = collection.purchases / collection.transactions * 100;
                        collection.amount_game_vs_unique_users = collection.amount_game / collection.n_unique_users_by_purchase;

                        tableTotal.purchases += collection.purchases;
                        tableTotal.transactions += collection.transactions;
                        tableTotal.unique_users += collection.unique_users;
                        tableTotal.amount_game_vs_unique_users += collection.amount_game_vs_unique_users;
                        tableTotal.amount_total += collection.amount_total;
                        tableTotal.amount_game += collection.amount_game;
                        tableTotal.n_unique_users_by_purchase += collection.n_unique_users_by_purchase;
                    });

                    tableTotal.cr = tableTotal.purchases / tableTotal.transactions * 100;
                    tableTotal.amount_game_vs_unique_users = tableTotal.amount_game / tableTotal.n_unique_users_by_purchase;

                    $scope.tableTotal = tableTotal;
