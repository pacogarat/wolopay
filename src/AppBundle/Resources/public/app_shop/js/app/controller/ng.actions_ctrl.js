angular.module('shopApp').controller('ActionsCtrl', ['$rootScope', '$scope', 'routing', 'pageTransition', 'alerts', '$http', '$sce', 'sliders', '$timeout', 'ArticleHelper', '$translate', '$log', 'ExternalStoreFacebook',
    function ($rootScope, $scope, routing, pageTransition, alerts, $http, $sce, sliders, $timeout, ArticleHelper, $translate, $log, ExternalStoreFacebook) {
        ...
        $scope.buy=function ()
        {
            ...
            var isIframe = false, isAjax = false, isServer2Server=false, url = null;
            ...
            if (!$rootScope.current.code)
            {
                ...
                isServer2Server = $rootScope.current.articlePMPCA.is_server2_server;
            ...
            if ($rootScope.options.externalStore_pmp)
            {
                isIframe = $rootScope.options.externalStore_pmp.is_iframe;
                isAjax = $rootScope.options.externalStore_pmp.is_ajax;
                isServer2Server = $rootScope.options.externalStore_pmp.is_server2_server;
            }
