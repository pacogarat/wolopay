angular.module('shopApp').controller('ArticlesListCtrl', [ 'APIAppShopHasArticles', 'APIArticlePMPCA', '$scope', '$rootScope', '$timeout', '$translate', 'state', '$filter', 'alerts', 'APIItemTab', '$log',
    function (APIAppShopHasArticles, APIArticlePMPCA, $scope, $rootScope, $timeout, $translate, state, $filter, alerts, APIItemTab, $log) {
        ...
        {
            if ($scope.addCartVisible(appShopHasArticle)){
        ...
        $rootScope.addCart = function (appShopHasArticle, event)
        {
            ...
            {
                //if (!event) to avoid being called twice (one from "activateArticle", without event)
                if (!event) alerts.addWarning('warnings.shopping_cart_max_items');
                return;
            }


            if ($rootScope.current.real_cart_price && $rootScope.options.shoppingCartMaxPriceEur <= $rootScope.current.real_cart_price.amount_eur){
                if (!event) alerts.addWarning('warnings.shopping_cart_max_price');
                return;
            }
