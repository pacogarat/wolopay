...
use AppBundle\Tests\E2E\PageActions\PayMethod\RixtyPage;

class RixtyPaymentTest extends AbstractE2EShop
{
    /**
     * @group E2E
     * @group Selenium
    ...
    public function testOK()
    {
        ...
        $apmpcs = $this->em->getRepository("AppBundle:AppShopArticleHasPMPC")->findByAppIdAndProviderNameAndCountryAndLevelCategoryandArticleIdAndStatus(
            $app->getId(), ProviderEnum::RIXTY_NAME, CountryEnum::USA, LevelCategoryEnum::ROOKIE_ID, $article->getId()
        );
        ...
        $page = new RixtyPage($this->driver);
        $page
        ...
        if ($page->isLogged() == false)
        {
            $page
                ...
                ->fillLoginAccountAndSend(
                    $this->container->getParameter('client_rixty_username'),
                    $this->container->getParameter('client_rixty_password')
                )
            ;
        }
