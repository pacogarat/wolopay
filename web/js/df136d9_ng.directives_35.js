...
angular.module('app.main', [])
    ...
    .directive('action', [ '$translate', function ($translate) {
        return {
            restrict: 'A',
            link: function (scope, element, attrs) {
                /*
                ...
                var smartActions = {
                    ...
                    userLogout: function ($this) {
                        ...
                        $translate(['yes', 'no', 'logout', 'logout_extra']).then(function(translations){
                            // ask verification
                            $.SmartMessageBox({
                                title: "<i class='fa fa-sign-out txt-color-orangeDark'></i> "+translations.logout+" <span class='txt-color-orangeDark'><strong>" + $('#show-shortcut').text() + "</strong></span> ?",
                                content: translations.logout_extra,
                    ...
                    resetWidgets: function ($this) {
                        ...
                        $translate(['yes', 'no', 'clear_storage', 'clear_storage_extra']).then(function(translations){

                            $.SmartMessageBox({
                                title: "<i class='fa fa-refresh' style='color:green'></i> "+translations.clear_storage,
                                content: $.widresetMSG || translations.clear_storage_extra,
                                buttons: '['+translations.no+']['+translations.yes+']'
                            }, function (ButtonPressed) {
                                if (ButtonPressed == translations.yes && localStorage) {
                                    localStorage.clear();
                                    location.reload();
                                }
            ...
            template: '\
				<li data-ng-class="{active: active}">\
					<a href="" title="{{title | translate}}">\
						...
						<span class="menu-item-parent" data-translate="{{ title }}" ng-cloak=""></span>\
					</a>\
                ...
                scope.$watch('active', function (newVal, oldVal) {
                    ...
                    if (newVal) {
                        ...
                        $translate(scope.title).then(function (titleTranslated){
                            $window.document.title = titleTranslated;
