smartApp.controller('ArticlesShopsController', ['APIStats', '$scope', '$rootScope', '$log', 'Sparkline', 'Flot', 'localize', '$timeout', '$filter',
    function (APIStats, $scope, $rootScope, $log, Sparkline, Flot, localize, $timeout, $filter) {
        ...
        {
            var pies = ['flotcontainer_shops_pie_only_articles', 'flotcontainer_shops_pie'];
        ...
        function exe()
        {
            APIStats.getArticlesShops().success(function (data){
                ...
                var xAsis=Flot.getXAxisFormatTime($scope.dateFormat), pieDrillDownObj = {}, shopsObj = {}, shopsPie=[],
                    tabsPie = [], articlesPie = [], onlyArticlesPie = [], onlyArticlesPieObj = {}, colors = Flot.getColors(), i =0;
                ...
                angular.forEach(onlyArticlesPieObj, function(article) {
                    onlyArticlesPie.push({y: Math.round(article.amount_game* 100) / 100, name: article.name, name_custom: article.id, color: Flot.increaseBrightness(article.color, 15)});
                });

                angular.forEach(pieDrillDownObj, function(shop, shopName) {
                    shopsPie.push({y: Math.round(shop.amount_game* 100) / 100, name: shopName, color: shop.color});
                    angular.forEach(shop.tabs, function(tab, tabName) {
                        tabsPie.push({y: Math.round(tab.amount_game* 100) / 100, name: tabName, color: Flot.increaseBrightness(shop.color, 30)});
                ...
                $timeout(function(){
                    ...
                    angular.forEach(shops, function(shop, shopName) {
                        ...
                        angular.forEach(shop.articles, function(articleBag) {
                            ...
                            name = $filter('api_translation')(articleBag.article.name_label, articleBag.article.items_quantity) + ',' + localize.localizeText('revenue');
