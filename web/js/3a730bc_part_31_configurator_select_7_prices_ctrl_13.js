smartApp.controller('ConfiguratorSelectPricesController', ['APIAppShopHasArticles', 'APIAppShops', 'APIApps', 'APICountries', 'APIArticles', 'APIPayMethods', '$scope', 'Utils', '$rootScope', '$http', 'Watchers', '$log', '$filter',
    function (APIAppShopHasArticles, APIAppShops, APIApps, APICountries, APIArticles, APIPayMethods, $scope, Utils, $rootScope, $http, Watchers, $log, $filter) {
                ...
                APIArticles.getArticleByAppId($rootScope.app.id, null, true, null, country.id).success(function (data){
                    ...
                    for (var i = 0; i < data.length; ++i) {
                        ...
                        for (var key = 0, length = articleSaved.app_shop_has_articles.length; key < length; ++key)
                        {
                            ...
                            console.log(key, appShopUpdated);
                            if (appShopUpdated.country.id == country.id)
                            {
                                for (var ii = 0; ii < data[i].app_shop_has_articles.length; ++ii)
                                {
                                    ...
                                    if (appShopHasArticleC.country.id == country.id && typeof appShopUpdated.current_amount_without_offer != 'undefined')
                                        appShopHasArticleC.current_amount_without_offer = appShopUpdated.current_amount_without_offer;

                                    if (appShopHasArticleC.country.id == country.id && typeof appShopUpdated.current_amount_virtual_without_offer != 'undefined')
                                        appShopHasArticleC.current_amount_virtual_without_offer = appShopUpdated.current_amount_virtual_without_offer;
        ...
        $scope.modifyPriceByArticleAmountVirtualStandard = function(article, typeForce) {
            ...
            for (var i = 0; i < $scope.countries.length; ++i)
            {
                ...
                for (var ii = 0; ii < $scope.appShops.length; ++ii)
                {
                    (function(){
                        ...
                        if (article.amount_virtual_standard === 0)
                        {
                            appShopHasArticle.current_amount_virtual_without_offer = 0;

                        }else{

                            if (($scope.autoConfiguration.cost_of_live_is_enabled && !typeForce) || typeForce == 'costOfLive')
                            {
                                $log.debug("costOfLive Virtual");
                                APICountries.getCostOfLifeVirtual(
                                    article.amount_virtual_standard, article.item.unitary_price_country.id, appShopHasArticle.country.id, $scope.autoConfiguration.pretty_price_is_enabled
                                ).success(function (data){
                                    appShopHasArticle.current_amount_virtual_without_offer = Math.round(parseFloat(data.price) * 100) / 100;
                                });

                            }else{

                                appShopHasArticle.current_amount_virtual_without_offer = article.amount_virtual_standard;
                            }
                        }
                    })();
                }
