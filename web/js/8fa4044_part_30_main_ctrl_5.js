smartApp
    ...
    .controller('SmartAppController',['$scope', '$rootScope', 'APIRoles', 'Permissions', '$route', '$animate', '$timeout', '$log', function ($scope, $rootScope, APIRoles, Permissions, $route, $animate, $timeout, $log) {
        ...
        $rootScope.$watch('dateQuickSelector', function(newValue, oldValue) {
            ...
            else if (newValue.indexOf('only_month_') != -1){

                var month =  parseInt(newValue.replace('only_month_','')-2 );
                dateFrom.date(1);
                dateTo.date(1);

                dateFrom.subtract(month+1, 'months');
                dateTo.subtract(month, 'months');
                dateTo.subtract('1', 'days');

            }else if (newValue.indexOf('last_month_') != -1){

                var month =  parseInt(newValue.replace('last_month_','')) - 1 ;
                dateFrom.subtract(month, 'months');
