        ...
        $scope.sendOffer = function ()
        {
            if ($rootScope.offerCurrent.update)
            {
                APIOffers.update($rootScope.app.id, $rootScope.offerCurrent.id, $rootScope.offerCurrent.name, Utils.getCSVFromSelectedObjectsId($rootScope.offerCurrent.app_shops),
                        ...
                        Utils.getCSVFromObjectId($rootScope.offerCurrent.articles_extra),
                    ...
                    .success(function (data){
                        ...
                        if (status == 422)
                        {
                            $translate('offer.conflicts_with_other_offer', {'%offer_name%': data.message.toString()}).then(function(translation){
                                alerts.addError(translation);
                            });
                        }
                ...
                APIOffers.insert($rootScope.app.id, $rootScope.offerCurrent.name, Utils.getCSVFromSelectedObjectsId($rootScope.offerCurrent.app_shops),
                    ...
                    .success(function (data){
                        ...
                        if (status == 422)
                        {
                            $translate('offer.conflicts_with_other_offer').then(function(translation){
                                alerts.addError(translation.replace('%offer_name%', data.message.toString()));
                            });
                        }
                    });
            }
