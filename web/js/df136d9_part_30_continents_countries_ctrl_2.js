smartApp.controller('ContinentCountriesController', ['APIStats', '$scope', '$rootScope', '$log', 'Sparkline', 'Flot', '$translate', '$timeout',
    function (APIStats, $scope, $rootScope, $log, Sparkline, Flot, $translate, $timeout) {
        ...
        $scope.reverse = true;
        $scope.showTableObjByDate = false;
            ...
            var exe = function exe()
            {
                APIStats.getContinentsCountries().success(function (data){
                    ...
                    angular.forEach(data.purchases_and_transactions_full, function(collection) {
                        ...
                        var name = collection.country_iso || 'XA', color = Flot.getColors()[i++];
                    ...
                    $scope.countryColors = countryColors;
        ...
        {
            var data = $scope.data;
            var countryColors = $scope.countryColors;
            var tableObj = {}, table = [], lastDate = '';

            if ($scope.showTableObjByDate == false)
            {
                ...
                angular.forEach(data.transactions_by_date, function(collection) {
                    ...
                    if (!tableObj[collection.country_iso])
                    {
                        tableObj[collection.country_iso] = {
                            country_iso: collection.country_iso,
                            country_id: collection.country_iso,
                    ...
                    if (!tableObj[collection.country_iso].dates[collection.country_iso])
                    {

                        tableObj[collection.country_iso].dates[collection.date_format] = {
                    ...
                    tableObj[collection.country_iso]['unique_users'] += collection.unique_users;
                    tableObj[collection.country_iso]['purchases'] += collection.purchases;
                    tableObj[collection.country_iso]['transactions'] += collection.transactions;
                    tableObj[collection.country_iso]['amount_game'] += collection.amount_game;
                    tableObj[collection.country_iso]['amount_total'] += collection.amount_total;

                    tableObj[collection.country_iso].dates[collection.date_format]['unique_users'] += collection.unique_users;
                    tableObj[collection.country_iso].dates[collection.date_format]['purchases'] += collection.purchases;
                    tableObj[collection.country_iso].dates[collection.date_format]['transactions'] += collection.transactions;
                    tableObj[collection.country_iso].dates[collection.date_format]['amount_game'] += collection.amount_game;
                    tableObj[collection.country_iso].dates[collection.date_format]['amount_total'] += collection.amount_total;
                });

                // Convert array map to array, we do this because angular filter search doesn't work with objects
                angular.forEach(tableObj, function(country) {
                    var temp = [];
                    country.cr = country.purchases / country.transactions * 100 ;
                    angular.forEach(country.dates, function(date) {
                        date.cr = date.purchases / date.transactions * 100 ;
                        temp.push(date);
                    });
