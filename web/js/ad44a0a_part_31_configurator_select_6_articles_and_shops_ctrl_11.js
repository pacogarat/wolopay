smartApp.controller('ConfiguratorSelectArticlesAndShopsController', ['APIAppShops', 'APILanguages', 'APIShopTemplates', 'APIArticles', 'APIPayMethods', '$scope', 'Utils', '$rootScope', 'APIAppTabs', 'APIAppShopHasAppTabs', '$filter', 'APIArticleCategories', 'APIPayCategories', '$modal', '$http', 'Watchers', 'LogTime', '$animate', '$timeout',
    function (APIAppShops, APILanguages, APIShopTemplates, APIArticles, APIPayMethods, $scope, Utils, $rootScope, APIAppTabs, APIAppShopHasAppTabs, $filter, APIArticleCategories, APIPayCategories, $modal, $http, Watchers, LogTime, $animate, $timeout) {
        ...
        {
            return name.replace(/ /g, '_').toLowerCase().substring(0, 45);
        }
            ...
            APIAppTabs.getByAppId($rootScope.app.id).success(function (data){
                ...
                APIAppShopHasAppTabs.getByAppId($rootScope.app.id).success(function (appShopHasAppTabs){
                    ...
                    APIAppShops.getByAppId($rootScope.app.id).success(function (selecteds){
                        ...
                        APIArticles.getArticleByAppId($rootScope.app.id, false, false, '1').success(function (articles){
        ...
        $scope.orderArticlesByAppShop = function (articles, appShop){
            ...
            for (var i = 0; i < articles.length; i++) {
                for (var ii = 0; ii < articles[i].app_shop_has_articles.length; ii++) {
                    if (articles[i].app_shop_has_articles[ii].app_shop.level_category.id == appShop.level_category.id)
                    {
                        articles[i].order = articles[i].app_shop_has_articles[ii].order;
                        break;
                    }
                }
            }
        ...
        $scope.someSelected = function (){
            ...
            for (var i = 0; i < $scope.categories.length; i++) {
                for (var ii = 0; ii < $scope.categories[i].app_shop_has_app_tabs.length; ii++) {
                    for (var iii = 0; iii < $scope.categories[i].app_shop_has_app_tabs[ii].articles.length; iii++) {
                        if ($scope.categories[i].app_shop_has_app_tabs[ii].articles[iii]['selected_'+$scope.categories[i].id])
                            return true;
                    }
                }
            }
