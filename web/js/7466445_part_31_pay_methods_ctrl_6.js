smartApp.controller('DocsPayMethodsController', ['$scope', 'APIPayMethods', '$rootScope',
    function ($scope, APIPayMethods, $rootScope) {
    ...
    $scope.getCurrenciesOfPayMethod = function (payMethod)
    {
        var result = [];
        var exist = false;
        angular.forEach(payMethod.pay_method_has_provider, function(payMethodHasProvider) {

            angular.forEach(payMethodHasProvider.pay_method_provider_has_countries, function(currency) {

                if (result.indexOf(currency.id) == -1)
                {
                    result.push(currency.country.id + " (" + currency.currency.id + ") - ");
                }
            });
